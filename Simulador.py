		...
		for tic in range(tics):
			...
			for t in range(len(cls.tortugas)):
				...
				if(tic % cls.transecto_berma[0][1] == 0):  ##cada cuento los contadores por transecto paralelo cuentan 
					if(posicion_tortuga[0] >= cls.contadores_TPB[0].obt_posicion()[0] and posicion_tortuga[0] <= cls.contadores_TPB[0].obt_posicion()[0]+30 and posicion_tortuga[1] >= cls.contadores_TPB[0].obt_posicion()[1]-30 and posicion_tortuga[1] <= cls.contadores_TPB[0].obt_posicion()[1]+30):
						cls.contadores_TPB[0].contar()
		cls.total_contadas_C()
		cls.total_contadas_TPB()
		cls.total_contadas_TVB()
		cls.total_tortugas_anidaron()
		arreglo_cantidades_totales_x_process = []
		arreglo_cantidades_totales_x_process.append(cls.total_tortugas_anidadas)
		arreglo_cantidades_totales_x_process.append(int(cls.formula_C(tics,tics//cls.cuadrantes[0][1])))
		arreglo_cantidades_totales_x_process.append(int(cls.formula_TVB(tics,tics//cls.transectos_verticales[0][1])))
		arreglo_cantidades_totales_x_process.append(int(cls.formula_TPB(tics//cls.transecto_berma[0][1],cls.transecto_berma[0][1])))
		return arreglo_cantidades_totales_x_process
